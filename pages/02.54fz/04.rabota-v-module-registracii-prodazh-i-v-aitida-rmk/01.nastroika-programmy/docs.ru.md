...
Вид оплаты «Кредит» по ФФД 1.05 и выше соответствует оплате «Постоплата (кредит)» (тэг 1216) и используется при отражении факта продажи товаров во внутреннюю рассрочку от магазина.

Вид оплаты «Сертификат» по ФФД 1.05 и выше соответствует оплате «Предоплата (аванс)» (тэг 1215) и используется при отражении факта получения предоплаты при продаже товаров. Самым ярким примером работы с предоплатой является работа с Подарочными картами (Сертификатами).

Вид оплаты «Прочее» по ФФД 1.05 и выше соответствует оплате «Встречное предоставление» (тэг 1217). Используется при отражении факта получения оплаты «встречным предоставлением» при продаже товара.
